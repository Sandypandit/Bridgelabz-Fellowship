...
public class LineComparison{
   public static void main(String[] arg){
	...
	DecimalFormat df = new DecimalFormat("#.00");	//limit after decimal point  
	  double length1 = Math.sqrt(Math.pow((x2-x1),2)+Math.pow((y2-y1),2));
	  double length2 = Math.sqrt(Math.pow((a2-a1),2)+Math.pow((b2-b1),2));
	  //double cannot be dereferenced, so converting to Double reference type
	...
	System.out.println("Length of the Line 2 is "+df.format(length2));
	    //comparing using java equals method
	if(L1.equals(L2)){ System.out.println("\ngiven two lines are equal");}
